  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Content */}
        <div className="relative z-10">
          <div className={cn("mb-4 transition-all duration-500", isHovered && "scale-110 rotate-12")}>
            <LeafIcon className="drop-shadow-lg" />
          </div>
          <h4
            className={cn("font-serif text-xl mb-2 transition-all duration-300", isHovered && "translate-x-1")}
          >
            {title}
          </h4>
          <p
  ...
  return (
    <section className="bg-white px-4 py-16">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="text-center mb-12 opacity-0 animate-[fadeIn_0.8s_ease-out_forwards]">
          <h2 className="font-serif text-3xl md:text-4xl mb-2 text-gray-900 tracking-wide">Why choose us?</h2>
          <p className="text-gray-600 text-lg">Designed For Your Peace Of Mind</p>
        </div>
        ...
        <div className="md:hidden relative flex items-center justify-center">
          <button
            onClick={() => scroll("left")}
            className="absolute left-4 top-1/2 -translate-y-1/2 bg-purple-700 text-white p-2 rounded-full z-10 shadow-md active:scale-90"
          >
            <ChevronLeft size={22} />
          </button>
          ...
          <div
            ref={scrollRef}
            className="flex gap-4 overflow-x-auto scrollbar-hide scroll-smooth px-2 snap-x snap-mandatory justify-center"
          >
            {cards.map((c, i) => (
              <div
                key={i}
                className="min-w-[90%] max-w-[90%] mx-auto snap-center rounded-2xl bg-gradient-to-br from-purple-800 via-purple-900 to-purple-950 
                           text-white p-8 shadow-xl flex-shrink-0
                           hover:shadow-2xl hover:shadow-purple-500/30 transition-all duration-300"
              >
                <LeafIcon className="mb-4" />
                <h4 className="font-serif text-xl mb-2">{c.title}</h4>
                <p className="text-purple-100/90 text-base leading-relaxed">{c.desc}</p>
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className="absolute right-4 top-1/2 -translate-y-1/2 bg-purple-700 text-white p-2 rounded-full z-10 shadow-md active:scale-90"
          >
